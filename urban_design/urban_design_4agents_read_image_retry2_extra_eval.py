...
class SummaryAction(Action):
    ...
    async def run(self, content: str = "", save_path: str = ""):
        ...
        try:
            ...
            # # 清空simplified_evaluations，防止每一轮叠加
            # simplified_evaluations = []
    ...
    async def _act(self) -> Message:
        ...
        if self.current_round > self.max_rounds+1:
            execution_logger.log_action(
                agent=self.name,
                action="Terminate",
                description="Final evaluation for maximum rounds reached, sending termination message",
                status="success"
            )
            summary_msg = Message(content="TERMINATE", role=self.profile, cause_by=type(todo), send_to=())
            self.publish_message(summary_msg)
            return summary_msg
...
async def main(
    ...
    while not env.is_idle:
        ...
        if summary_agent.current_round > max_rounds+1:
            logger.info("Final evaluation for maximum rounds reached, stopping environment")
            break
